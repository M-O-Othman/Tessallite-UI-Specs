...
Verification: `npm run check` passes all 111 tests in 13 files, including maximise/restore, Escape, resize/refit, wheel panning, embedded primary logo, compact header status and white surfaces under dark OS preference. TypeScript unused-local/parameter checks pass for core, CLI and MCP. The private Excel document validates; its exported page loads and supports handler search/selection and maximise/Escape in the DOM test environment. All eight MCP tools passed over stdio. No actual Chrome rendering or narrow-screen visual claim is made.
